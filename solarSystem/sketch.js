...
function setup() {
  ...
  zoomIn.position(100, 100);
  zoomIn.size(30,30);
  zoomOut.position(100,130);
  zoomOut.size(30,30);
  zoomIn.mousePressed(zoomPageIn);
  zoomOut.mousePressed(zoomPageOut);
  zoomLevel = 1;
  
  speedSlider = createSlider(0,10,1);
  speedSlider.position(135,120);
  
  pauseButton = createButton("pause");
  pauseButton.mousePressed(pause);
  pauseButton.position(width/2, 100);
  pauseButton.size(60,30);
  playButton = createButton("play");
  playButton.mousePressed(play);
  playButton.position(width/2+70, 100);
...
function mousePressed() {
  ...
  // ensure mouse is not overlapping any buttons!
  if (mouseX > 80 && mouseX < 1000 && mouseY > 80 && mouseY < 180) {
    print("print me");
  } else {
    solarSystem.addMusicLine(mouseX - width/2,mouseY - height/2);
  }
...
function mouseDragged() {
  if (mouseX > 80 && mouseX < 880 && mouseY > 80 && mouseX < 180) {
  } else {
...
//  1 (m) : 2.41 (v) : 2.54 (e) : 1.34 (m) : 28.4 (j) : 24 (s) : 10.2 (u) : 9.88 (n) : 1/2.18 (p)
  ...
  var orbitScaleFactor = 2;
  // sun = new Planet(10,0,0,0);
  mercury = new Planet("mercury", 1*sizeScaleFactor,0,0.206,5.79, 2);
  venus = new Planet("venus", 2.41*sizeScaleFactor,0,0.0068,10.8, 5.11);
  earth = new Planet("earth", 2.54*sizeScaleFactor,0,0.0167,15, 8.3);
  mars = new Planet("mars", 1.34*sizeScaleFactor, 0, 0.0934, 22.8, 15.61);
  jupiter = new Planet("jupiter", 28.4*sizeScaleFactor, 0, 0.0485, 77.8, 98.05);
  saturn = new Planet("saturn", 24,0*sizeScaleFactor, 0.0556, 143, 244);
  uranus = new Planet("uranus", 10.2,0*sizeScaleFactor, 0.0472, 287, 696);
  neptune = new Planet("neptune", 18*sizeScaleFactor,0, 0.0086, 450, 1360);
  pluto = new Planet("pluto", 0.0022*2.54*sizeScaleFactor, 0, 0.25, 590, 2048);
}
...
Planet.prototype.display = function() {
  ...
  text(this.name, this.x + this.scaledSize/2 + 5,this.y);
...
Planet.prototype.checkForMusicLine = function() {
  for (var i = 0; i < solarSystem.musicLines.length; i++) {
    if (this.theta - solarSystem.musicLines[i].theta < 0.01 && this.theta - solarSystem.musicLines[i].theta > -0.03) {
